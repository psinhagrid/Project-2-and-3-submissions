...
def df_splitter_and_vectorizer(df):
            ...
            X_train, X_test, y_train, y_test = train_test_split(df['preprocessed_text'],                                           # Split data to train and test
                                                    df[['answer_type','metadata_language', 'metadata_skills', 'metadata_task',     # Input is preprocessed_text, output is different classes
                                                        'metadata_category', 'metadata_context', 'metadata_grade',
                                                        'metadata_split', 'metadata_source', 'metadata_img_height', 
                                                        'metadata_img_width']], 
                                                    test_size=0.3, random_state=42)          # Test size is 30 %
            
   

            # Fit the vectorizer on the preprocessed text data and transform it
            X_train_tfidf = tfidf_vectorizer.fit_transform(X_train)       # Vectorising Training data

            X_test_tfidf = tfidf_vectorizer.transform(X_test)             # transforming testing vectors 

            return X_train_tfidf, X_test_tfidf, y_train, y_test           # Return vectorized train and test data
...
def get_df_paths():
    ...
    parent_directory = os.path.dirname(current_directory)       # Going to parent directory                                 
    

    # Navigate to the "data" folder inside the parent directory
    data_directory = os.path.join(parent_directory, "data")     # Going inside data directory                                                                          

    # Navigate to the "processed" subfolder inside the "data" folder
    processed_directory = os.path.join(data_directory, "processed")   # Going inside processed folder          

    

    # Define the paths to the data files in a list
    csv_file_paths = [
...
csv_file_paths = get_df_paths()         # Call the functions
...
print ("Data Frame Obtained")       # Confirm to user
...
def classifier_accuracy(rf_classifier, category):
    ...
    """
        This function will give the accuracy of each class and print it. 
    """
    # Train the classifier
    rf_classifier.fit(X_train_tfidf, y_train[category])             # Fit on category asked by user

    # Predict the labels for the test set
    y_pred = rf_classifier.predict(X_test_tfidf)                    # Make predictions

    # Evaluate the classifier
    accuracy = accuracy_score(y_test[category], y_pred)
    print("Accuracy of " + category + ":", accuracy)                # Print accuracy
...
def best_SVM_Hyperparameters(model_name, accuracy_metric):
    ...
    """
        ...
        This will work for both SVM and Naive Bayes
    """
    ...
    # Grid search is done for either SVM or Naive Bayes.
    for i in range(len(metadata_features_list)):
        param_grid = {}
        if model_name == "SVM":
            param_grid = {'C': [0.1, 1, 10], 'kernel': ['linear', 'poly', 'rbf'], 'degree': [1, 2, 3, 4]}   # Parameters which can be changed
            model = svm.SVC()
        elif model_name == "NB":
...
def plot_accuracy_vs_degree(class_name):
    ...
    """
    We can also Visualize which degree is most suitable for SVM. 
    We do so by plotting a graph between accuracy and degree. 
...
def SVM_plotter():
    """
    Helps to visualize our SVM model after reducing dimension using lsa
...
def evaluation_of_model(model_name, accuracy_metric):
    ...
    It can evaluate either SVM or Naive Bayes
    It also prints our reportfor each class 
    ...
    X_train_tfidf, X_test_tfidf, y_train, y_test = df_splitter_and_vectorizer(df)   # Vectoring our data
